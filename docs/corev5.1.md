# CORE v5.1 — Fluxo de estudo por blocos e capítulos (oficial)

Changelog v5.1 (2026-01-29)

- Formaliza escopo provisório quando a ementa do capítulo ainda não existe (início de capítulo).
- Endurece requisitos de formatação (provas e correções em Markdown copiável e organizado).
- B6: exige pesos por dificuldade + anti-cópia + anti-tutorial (avaliar compreensão).
- B1/B4/B7: parecer/correção com densidade mínima e estrutura padrão.
...
Para qualquer prova (B3/B6/C4) e qualquer correção (B4/B7/C6), eu devo entregar em Markdown copiável, com:

- Título + identificador (ex.: `B6 — C04B02`).
- Seções fixas quando aplicável: `Instruções`, `Questões`, `Pontuação`, `Critérios`, `Checklist`.
- Numeração consistente (Q1..Qn) e, quando houver, subitens (a), (b), (c).
...
1. `ementa.md` (OFICIAL, único/geral): documento único com seções por capítulo (C03, C04, ...), contendo o que foi aprendido e pode ser cobrado.
...
Padrão mínimo de entrega (B4) — em Markdown copiável:
...
- Eu não peço ementa repetidamente sem necessidade.
...
- Cada prova deve conter, no mínimo:
  ...
  - 1 questão de “aplicar em variação” (mesmo conceito em contexto diferente: shapes/dtypes/casos-limite).
...
Padrão mínimo de entrega (B7) — em Markdown copiável:
